...
class OptimizedCollaborativeFilter:
    ...
    def _compute_item_similarity(
        self, use_cache: bool = True, min_interactions: int = 5
    ) -> np.ndarray:
        """
        ...
        if valid_items.sum() < self.matrix.shape[1]:
            print(
                f" Filtering items: {valid_items.sum()}/{self.matrix.shape[1]} items have ≥{min_interactions} interactions"
            )
    ...
    def _compute_similarity_chunked(
        self, matrix: csr_matrix, chunk_size: int = 1000
    ) -> np.ndarray:
    ...
    def recommend_items_optimized(
        ...
        if user_index >= self.matrix.shape[0]:
            raise ValueError(
                f"User index {user_index} out of range (max: {self.matrix.shape[0]-1})"
            )
        ...
        if user_ratings.nnz == 0:
            print("  User has no ratings, returning popular items...")
        ...
        print("  Computing predicted ratings...")
        ...
        scores[valid_predictions] = (
            numerator[valid_predictions] / denominator[valid_predictions]
        ) + user_mean
